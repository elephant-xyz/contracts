...
describe("VMahout", function () {
    ...
    beforeEach(async function () {
        ...
        vMahout = (await upgrades.deployProxy(
            VMahoutFactory,
            [await admin.getAddress(), await minter.getAddress(), await upgrader.getAddress()],
    ...
    describe("Initialization", function () {
        ...
        it("should set the correct admin, minter, and upgrader roles", async function () {
            ...
            expect(await vMahout.hasRole(DEFAULT_ADMIN_ROLE, await admin.getAddress())).to.be.true;
            expect(await vMahout.hasRole(MINTER_ROLE, await minter.getAddress())).to.be.true;
            expect(await vMahout.hasRole(UPGRADER_ROLE, await upgrader.getAddress())).to.be.true;
        });
    });

    describe("Token Properties", function () {
        it("should prevent transfers", async function () {
            await expect(vMahout.connect(user1).transfer(await user2.getAddress(), 100))
                .to.be.revertedWithCustomError(vMahout, "VMahout__TransferNotAllowed");
        });

        it("should prevent transferFrom", async function () {
            await expect(vMahout.connect(user1).transferFrom(await owner.getAddress(), await user2.getAddress(), 100))
                .to.be.revertedWithCustomError(vMahout, "VMahout__TransferNotAllowed");
        });

        it("should prevent approvals", async function () {
            await expect(vMahout.connect(user1).approve(await user2.getAddress(), 100))
                .to.be.revertedWithCustomError(vMahout, "VMahout__TransferNotAllowed");
        });

        it("should be upgradeable", async function () {
            const VMahoutV2Factory = await ethers.getContractFactory("VMahout"); // Using same for test simplicity
            const vMahoutV2 = await upgrades.upgradeProxy(await vMahout.getAddress(), VMahoutV2Factory.connect(upgrader));
    ...
    describe("Minting Logic", function () {
        it("should allow minter to mint tokens", async function () {
            ...
            expect(await vMahout.balanceOf(await user1.getAddress())).to.equal(amount);
        ...
        it("should not allow non-minter to mint tokens", async function () {
            ...
            await expect(vMahout.connect(user1).mint(user1Address, amount))
                .to.be.revertedWithCustomError(vMahout, "AccessControlUnauthorizedAccount")
        ...
        it("should mint correct amount of tokens", async function () {
            ...
            await vMahout.connect(minter).mint(await user1.getAddress(), firstMintAmount);
            expect(await vMahout.balanceOf(await user1.getAddress())).to.equal(firstMintAmount);

            const secondMintAmount = ethers.parseEther("300");

            await vMahout.connect(minter).mint(await user2.getAddress(), secondMintAmount);
            expect(await vMahout.balanceOf(await user2.getAddress())).to.equal(secondMintAmount);
